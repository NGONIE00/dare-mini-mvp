...
const FEATURES = [
  {
    ...
    desc: "Works via USSD on basic handsets. No smartphone, no app store, no data plan required. If you can make a call, you can use Dare.",
    color: "#D97706",
    icon: (
  ...
  },
  {
    ...
    desc: "Audio quality scales to your connection. Optimised for minimum data use — fully intelligible even on 2G networks.",
    color: "#059669",
    icon: (
  ...
  },
  {
    ...
    desc: "Hosts control who speaks, set chat etiquette rules, mute participants, and manage the room queue — keeping sessions focused and respectful.",
    color: "#7C3AED",
    icon: (
  ...
  },
  {
    ...
    desc: "Host and listen in your own language. Prioritising local languages with a roadmap to support 20+ languages globally.",
    color: "#3B82F6",
    icon: (
  ...
  },
  {
    ...
    desc: "Pay creators via local mobile money. No bank account required. ~85% of every payment goes directly to the creator.",
    color: "#D97706",
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    desc: "Creators own their audience data. No algorithmic manipulation, no shadow banning. What you build is yours to keep.",
    color: "#EF4444",
    icon: (
  ...
  },
  {
    ...
    desc: "Sessions buffer automatically. Listen when your signal returns — no rejoining, no missed content.",
    color: "#059669",
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
export default function Home() {
  ...
  /* ── style tokens ── */
  const sec:    React.CSSProperties = { background: "var(--bg)",  borderBottom: "1px solid var(--divider)", padding: "3.5rem 2rem", transition: "background 0.3s, border-color 0.3s" };
  const secAlt: React.CSSProperties = { background: "var(--bg2)", borderBottom: "1px solid var(--divider)", padding: "3.5rem 2rem", transition: "background 0.3s, border-color 0.3s" };
      ...
      <section style={sec}>
        <div style={inner}>
          ...
          <div style={{ background: "var(--bg2)", borderRadius: "0 10px 10px 0", borderLeft: "3px solid #D97706", overflow: "hidden", transition: "background 0.3s" }}>
              ...
              <div style={{ display: "flex", transform: `translateX(-${featCur * 100}%)`, transition: "transform 0.42s cubic-bezier(.4,0,.2,1)" }}>
                {FEATURES.map((f, i) => (
                  <div key={i} style={{ minWidth: "100%", padding: "1.25rem 1.5rem", display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                    ...
                    <div>
                      <span style={{ fontSize: "0.63rem", fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", padding: "2px 8px", borderRadius: 100, display: "inline-block", marginBottom: "0.35rem", background: "var(--bg)", color: "var(--text2)", border: "0.5px solid var(--border2)", fontFamily: "sans-serif", transition: "background 0.3s, color 0.3s" }}>{f.tag}</span>
                      <div style={{ fontSize: "0.95rem", fontWeight: 700, color: "var(--text)", marginBottom: "0.3rem", transition: "color 0.3s" }}>{f.title}</div>
                      <div style={{ fontSize: "0.8rem", color: "var(--text2)", lineHeight: 1.65, fontFamily: "sans-serif", transition: "color 0.3s" }}>{f.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Dots */}
            <div style={{ display: "flex", gap: 6, padding: "0.75rem 1.5rem", alignItems: "center" }}>
              {FEATURES.map((_, i) => (
      ...
      <section style={secAlt}>
        <div style={inner}>
          ...
          <div style={{ background: "var(--bg)", border: "0.5px solid var(--border)", borderRadius: 10, padding: "1.25rem", marginTop: "1.25rem", transition: "background 0.3s, border-color 0.3s" }}>
            ...
            <div style={{ fontSize: "0.83rem", fontWeight: 600, color: "var(--text)", marginTop: "0.5rem", fontFamily: "sans-serif", transition: "color 0.3s" }}>{BW_DATA[bwVal-1].label}</div>
          </div>
        </div>
      </section>
      ...
      <footer style={{ background: "var(--bg)", borderTop: "1px solid var(--divider)", padding: "2.25rem 2rem 1.5rem", transition: "background 0.3s, border-color 0.3s" }}>
        <div style={inner}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(110px,1fr))", gap: "1.25rem", marginBottom: "1.5rem" }}>
            ...
            {[
              { h: "Product", links: [{ l: "Browse rooms", fn: () => router.push("/rooms") }, { l: "Get started", fn: () => router.push("/register") },{ l: "Feature phone (*447#)", fn: () => router.push("/ussd") },{ l: "Impact metrics",fn: () => router.push("/analytics") },] },
            ...
            <div style={{ padding: "0.75rem 1.4rem" }}>
              {filteredRooms.map((r, i) => (
                <div key={i} style={{ border: "0.5px solid var(--border)", borderRadius: 8, padding: "0.9rem", marginBottom: "0.65rem", background: "var(--bg)", transition: "background 0.3s" }}>
                  ...
                  {r.status === "live" ? (
                    <button onClick={() => { setJoined(j => ({ ...j, [i]: true })); router.push("/rooms"); }} style={{ background: joined[i] ? "#059669" : "#D97706", color: "#fff", border: "none", borderRadius: 4, padding: "5px 14px", fontSize: "0.73rem", fontWeight: 600, cursor: "pointer", fontFamily: "sans-serif", transition: "background 0.3s" }}>
